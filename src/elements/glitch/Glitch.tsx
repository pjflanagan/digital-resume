...
const generateGlitchConfig = (phase: number, strength: 'weak' | 'medium' | 'strong') => {
...
function Glitch({ children, glitchIndex, className, strength = 'medium' }: GlitchProps): ReactNode {
  ...
  const [prevGlitchIndex, setPrevGlitchIndex] = useState<unknown>(INITIAL_TRIGGER);
  const [prevStrength, setPrevStrength] = useState<string>(strength);

  if (glitchIndex !== prevGlitchIndex || strength !== prevStrength) {
    setPrevGlitchIndex(glitchIndex);
    setPrevStrength(strength);
    setGlitchState(generateGlitchConfig(1, strength));
  }
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => {
      setGlitchState(generateGlitchConfig(2, strength));
    }, 60);
    timers.push(t2);

    // Phase 3: Glitch after 120ms
    const t3 = setTimeout(() => {
      setGlitchState(generateGlitchConfig(3, strength));
    }, 120);
    timers.push(t3);
