...
type BodyState = {
  pos: Point;
  offset: { x: number; y: number; to: Point };
  colorPos: {
    angle: number;
...
abstract class Body {
  ...
  constructor(canvas: View, layer: number, id: number) {
    ...
    // variables
    this.prop = {} as BodyProp;
    this.state = {} as BodyState;
    ...
    this.state.offset = { x: 0, y: 0, to: { x: 0, y: 0 } };
    this.setOffsetTo();
  }
...
class Ship extends Body {
  ...
  getShipScrollShiftedCenter(): Point {
